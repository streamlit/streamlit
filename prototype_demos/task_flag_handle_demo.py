...
"""Demo app for the `task=True` flag returning a handle (Alternative B).

Run with `make debug prototype_demos/task_flag_handle_demo.py`, then open the app
in two browser tabs with the same region selected to watch one shared computation
update both of them.
"""
...
st.title("Non-blocking cached execution — `task=True` returning a handle")
st.caption(
    "Alternative B: the function is declared with `task=True`, so calling "
    "`slow_query(region)` returns a handle instead of blocking."
)
...
st.write("Cached already:", slow_query(region).done)
