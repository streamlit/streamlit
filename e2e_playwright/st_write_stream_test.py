...
def test_stream_incomplete_bold(app: Page, assert_snapshot: ImageCompareFunction):
    """Test that incomplete bold markdown is correctly styled during streaming.

    This tests the remend integration which completes incomplete markdown syntax.
    """
    click_button(app, "Stream incomplete bold")

    # Wait for the stream to complete - the text should be visible
    stream_output = get_element_by_key(app, "stream-output")
    expect(stream_output.get_by_test_id("stMarkdown")).to_be_visible()

    # The bold text should be styled correctly (remend completes the **)
    bold_element = stream_output.locator("strong")
...
def test_stream_incomplete_code_block(app: Page, assert_snapshot: ImageCompareFunction):
    """Test that incomplete code block is correctly styled during streaming.

    This tests the remend integration which completes incomplete markdown syntax.
    """
    click_button(app, "Stream incomplete code")
