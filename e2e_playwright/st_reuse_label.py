...
# Initialize session state for test_run_count
if "test_run_count" not in st.session_state:
    st.session_state.test_run_count = 0 if st.get_option("server.headless") else -1
...
if st.session_state.test_run_count < 1:
    w1 = st.slider("label", 0, 100, 25, 1)
else:
    w1 = st.selectbox("label", ("m", "f"), 1)

st.write("value 1:", w1)
st.write("test_run_count:", st.session_state.test_run_count)
st.write(
    """
