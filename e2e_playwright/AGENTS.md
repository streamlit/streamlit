...
- **Media/binary files** (images, audio, video, PDFs): Place in `e2e_playwright/static/` and load via file path (e.g., `STATIC_DIR / "cat.jpg"`) or URL (`./app/static/<filename>`).
- **Non-media config files** (secrets, HTML templates, JS scripts): Place in `e2e_playwright/test_assets/`.
