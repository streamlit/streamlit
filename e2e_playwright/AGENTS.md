...
- **All test assets** (images, audio, video, PDFs, config files, HTML templates, JS scripts): Place in `e2e_playwright/static/` and load via file path (e.g., `STATIC_DIR / "cat.jpg"`) or URL (`./app/static/<filename>`).
