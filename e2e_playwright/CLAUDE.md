...
- `make debug-e2e-test` - Run a playwright e2e test in debug mode. Use it via `make debug-e2e-test st_command_test.py`.
- `make run-e2e-test` - Run a playwright e2e test. Use it via `make run-e2e-test st_command_test.py`.
- `make update-snapshots` - Update e2e playwright snapshots based on the latest completed CI run.
