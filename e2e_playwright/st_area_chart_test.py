...
def test_area_chart_alt_sets_accessible_name(app: Page):
    """`alt` becomes the Vega graphics-document accessible name."""
    labeled = get_vega_graphics_document(get_element_by_key(app, "area_alt"))
    expect(labeled).to_have_accessible_name("Area chart of columns a, b, and c")

    unlabeled = get_vega_graphics_document(get_element_by_key(app, "area_no_alt"))
