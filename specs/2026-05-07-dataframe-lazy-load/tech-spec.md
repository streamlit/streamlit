...
- Enforce an LRU limit for random-access chunks.
...
```sql
CREATE TEMP TABLE __st_dataframe_<source_id> AS
SELECT
  ROW_NUMBER() OVER (
    ORDER BY <stable_sort_cols>, <unique_tie_breaker>
  ) - 1 AS __st_row_num,
...
- `on_select != "ignore"`: raise a clear `StreamlitAPIException`
...
- Client-side table-wide sort/search: disable for lazy dataframes. Existing large eager
  dataframes still show search, so auto-lazy must intentionally change that behavior or wait
