...
Add a single `resolution` parameter that accepts either an integer (single dimension) or a
`Literal` preset:
...
def camera_input(
    ...
    # ... existing parameters ...
    *,
    resolution: int | Literal["480p", "720p", "1080p"] | None = None,
) -> UploadedFile | None:
```
...
| `int` (e.g., `720`) | Target height in pixels | Camera targets this height; width determined by native aspect ratio |
| `"480p"` | 480 pixel height preset | Equivalent to `resolution=480` |
| `"720p"` | 720 pixel height preset | Equivalent to `resolution=720` |
| `"1080p"` | 1080 pixel height preset | Equivalent to `resolution=1080` |
...
picture = st.camera_input("Scan QR code", resolution=720)
if picture:
    from PIL import Image
    img = Image.open(picture)
...
# Lower resolution for faster uploads
...
| Invalid `resolution` value | `StreamlitAPIException` at call time |
| `resolution <= 0` | `StreamlitAPIException` at call time |

### Presets Rationale

Including `"480p"`, `"720p"`, and `"1080p"` presets provides:

- **Discoverability**: Users see common options in documentation and IDE autocomplete
- **Best practice guidance**: Users learn what resolutions are typically supported
- **Readability**: `resolution="720p"` is clearer than `resolution=720`

We intentionally omit `"4k"` / `"2160p"` because:
- Most webcams don't support 4K
- 4K images are large (~8MB+) and slow to upload
- Bandwidth impact is significant for Streamlit's use cases
...
### Option D: `resolution` as single dimension (RECOMMENDED)

```python
st.camera_input("Label", resolution=720)
...
- **`4k` / `"2160p"` preset**: Can add later if user demand exists and bandwidth concerns are
  addressed
