...
See the [product spec](product-spec.md) for the full problem description and API design.

## Proposal

### Background: `bind="query-params"` Execution Flow

The `bind="query-params"` feature syncs a widget's value to the URL query string:

```python
sort = st.selectbox("Sort by", ["name", "price", "rating"],
                    key="sort", bind="query-params")
# URL becomes ?sort=price when user selects "price"
# Sharing/bookmarking the URL restores the selection
```

Its *primary* purpose is URL syncing, but as a side effect it also preserves widget values
across conditional rendering and page switches — the same problem `persist_state` solves.
This makes `bind="query-params"` a functional workaround for widget persistence today, but
with significant drawbacks: values are exposed in the URL, URL length limits apply, browser
history is polluted on every interaction, and sharing the URL shares the widget state. The
`persist_state` parameter gives developers the preservation behavior without the URL side
effects.

What matters for this spec is not the URL syncing, but the **state preservation mechanism**
that makes it work. The `persist_state` implementation reuses this mechanism directly.

**Core idea:** A widget's value is stored in `_old_state` under one of two key types: an
internal *widget ID* (auto-generated, position-dependent) or a *user key* (the `key=`
parameter). Values arriving from the frontend use the widget ID; values set programmatically
use the user key. Normal stale cleanup removes widget-ID entries. The `bind="query-params"`
pattern intercepts cleanup to *re-key* the value: it reads the value out by widget ID, then
writes it back under the user key. When the widget re-registers on the next run, it finds
its preserved value via the user key lookup path.

The pattern has three phases:

**Phase 1 — Registration.** When a widget calls `register_widget` with `bind="query-params"`,
the widget ID is added to a durable set (`_query_param_bound_widget_ids`) that persists
across runs. This set is necessary because by the time stale cleanup runs, the widget's
metadata may already be gone (e.g., after an MPA page switch).
...
class SessionState:
    ...
    def register_widget(self, metadata, user_key):
        if metadata.bind == "query-params" and user_key is not None:
            self._query_param_bound_widget_ids.add(metadata.id)
        elif metadata.bind is None and user_key is not None:
            self._query_param_bound_widget_ids.discard(metadata.id)
```

**Phase 2 — Pre-cleanup capture.** In `_remove_stale_widgets`, *before* any state is deleted,
the method iterates stale widgets and checks the durable set. For any stale widget that
is bound, its current value is captured under the user key.
...
**Phase 3 — Cleanup + re-injection.** Normal stale cleanup runs, deleting widget-ID entries
from `_old_state` and `_new_widget_state`. Then the captured values are written back under
user keys.
...
The re-injected user-key entry serves two purposes on the next `register_widget` call:
1. **Value restoration:** `register_widget` checks `user_key not in self` to decide whether
   to use the default. Because the user key is in `_old_state`, the widget picks up the
   preserved value instead of resetting.
2. **Query param re-sync** (specific to `bind="query-params"`): `register_widget` checks
   `user_key in self._old_state` as a guard to detect values preserved by stale cleanup,
   and writes the value back to the URL if the query param is missing.

For `persist_state`, only purpose (1) applies — there is no URL to sync.

The `persist_state` implementation reuses this same capture-cleanup-reinject pattern,
extending the "should preserve?" check to include persisted widgets alongside
query-param-bound ones.
