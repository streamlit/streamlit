...
| Scope rule | Direct attachment (`on_change=sig` / `on_click=fragment_fn`) scopes the rerun to the watcher queue, suppressing the full rerun **and** the enclosing fragment's rerun. `send()` inside a plain callback or fragment body never changes whether the pass is a full run or a scoped pass — within a scoped pass it can only append watchers. | Suppression is the recoverable default: opting back in is one explicit step (make the enclosing fragment a watcher); opting out of an always-included enclosing fragment would be impossible. |
...
| Watcher resolution | At fire time, against currently-registered fragments | A watcher behind a false conditional is simply absent. A `send()` with zero watchers updates state and is otherwise a no-op; a *directly attached* widget whose signal has zero registered watchers falls back to a **full rerun** so the interaction's widget value isn't lost. |
...
still in flight, so nested containers stay correct.) A `parallel=True` fragment whose
*own* widget (or `run_every`) triggered the rerun is not a watcher in that pass — it is
the rerun's direct target and runs inline on the script thread, so interaction-gated
`st.dialog`/`st.switch_page` inside it keep working exactly as the parallel-fragments
spec promises. See *Out of Scope* for `st.wait()`, a
