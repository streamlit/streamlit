...
- Parameterized Tests: Use `@pytest.mark.parametrize` whenever it is possible to combine overlapping tests with varying inputs. Legacy tests may still use `@parameterized.expand`, but new tests should prefer the native pytest approach.
