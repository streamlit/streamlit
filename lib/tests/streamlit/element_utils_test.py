...
from streamlit.lib.elements.utils import check_callback_rules, check_session_state_rules
from streamlit.lib.errors import StreamlitAPIException


class ElementUtilsTest(unittest.TestCase):
    @patch("streamlit.lib.elements.utils.is_in_form", return_value=False)
    @patch("streamlit._is_running_with_streamlit", new=True)
    def test_check_callback_rules_not_in_form(self, _):
        check_callback_rules(None, lambda x: x)

    @patch("streamlit.lib.elements.utils.is_in_form", return_value=True)
    @patch("streamlit._is_running_with_streamlit", new=True)
