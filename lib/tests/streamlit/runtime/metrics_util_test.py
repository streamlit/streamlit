...
class MetricsUtilTest(unittest.TestCase):
    ...
    def test_machine_id_v3_from_etc(self):
        ...
        with patch("streamlit.runtime.metrics_util.uuid.getnode", return_value=MAC), patch(
            "streamlit.runtime.metrics_util.open", mock_open(read_data=file_data), create=True
        ), patch("streamlit.runtime.metrics_util.os.path.isfile") as path_isfile:
    ...
    def test_machine_id_v3_from_node(self):
        ...
        with patch("streamlit.runtime.metrics_util.uuid.getnode", return_value=MAC), patch(
            "streamlit.runtime.metrics_util.os.path.isfile", return_value=False
        ):
