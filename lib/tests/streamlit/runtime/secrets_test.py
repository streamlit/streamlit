...
class SecretsDirectoryTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        with open(
            os.path.join(self.temp_dir_path, "example_login", "username"), "w", encoding="utf-8"
        ) as f:
            f.write("example_username")
        with open(
            os.path.join(self.temp_dir_path, "example_login", "password"), "w", encoding="utf-8"
        ) as f:
            f.write("example_password")
        os.makedirs(os.path.join(self.temp_dir_path, "example_token"))
        with open(os.path.join(self.temp_dir_path, "example_token", "token"), "w", encoding="utf-8") as f:
            f.write("token123")
