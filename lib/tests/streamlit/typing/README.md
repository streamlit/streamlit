...
Intentional invalid calls use `# type: ignore[...]` for mypy and `# ty: ignore[...]` for ty. A valid call whose asserted type mypy accepts but ty rejects may use `# ty: ignore[type-assertion-failure]`; add a short note saying what ty infers instead, so the suppression can be removed once ty catches up.
