...
"""streamlit.lib.credentials unit test."""
import os
import textwrap
import unittest
...
from streamlit.lib.credentials import _Activation
from streamlit.lib.credentials import Credentials
from streamlit.lib.credentials import _verify_email

PROMPT = "streamlit.lib.credentials.click.prompt"
...
class CredentialsClassTest(unittest.TestCase):
    ...
    @patch("streamlit.lib.credentials.file_util.get_streamlit_file_path", mock_get_path)
    ...
    def test_Credentials_load(self):
        ...
        with patch("streamlit.lib.credentials.open", m, create=True):
            c = Credentials.get_current()
            c.load()
    ...
    def test_Credentials_load_twice(self):
        ...
        with patch("streamlit.lib.credentials.LOGGER") as p:
            c.load()
    ...
    def test_Credentials_load_file_not_found(self):
        ...
        with patch("streamlit.lib.credentials.open") as m:
    ...
    def test_Credentials_check_activated_already_loaded(self):
        ...
        c = Credentials.get_current()
        c.activation = _Activation("some_email", True)
        with patch("streamlit.lib.credentials._exit") as p:
    ...
    def test_Credentials_check_activated_error(self):
        ...
        with patch.object(c, "load", side_effect=Exception("Some error")), patch(
            "streamlit.lib.credentials._exit"
        ) as p:
            c._check_activated(auto_resolve=False)
    ...
    def test_Credentials_save(self):
        ...
        with patch(
            "streamlit.lib.credentials.open", mock_open(), create=True
        ) as open, patch("streamlit.lib.credentials.os.makedirs") as make_dirs:
    ...
    def test_Credentials_reset(self):
        ...
        with patch("streamlit.lib.credentials.os.remove") as p:
            Credentials.reset()
    ...
    def test_Credentials_reset_error(self):
        ...
        with patch(
            "streamlit.lib.credentials.os.remove", side_effect=OSError("some error")
        ), patch("streamlit.lib.credentials.LOGGER") as p:
