...
class ProxyUtilTest(unittest.TestCase):
    ...
    def test_url_is_from_allowed_origins_allowed_domains(self):
        with patch('streamlit.proxy.proxy_util.config.get_option',
                   side_effect=[True, None, None]) as m:
    ...
    def test_url_is_from_allowed_origins_CORS_off(self):
        with patch('streamlit.proxy.proxy_util.config.get_option', side_effect=[False]) as m:
            self.assertTrue(url_is_from_allowed_origins('does not matter'))

    def test_url_is_from_allowed_origins_s3_bucket(self):
        with patch('streamlit.proxy.proxy_util.config.get_option',
                   side_effect=[True, 'mybucket', 's3.amazon.com']) as m:
    ...
    def test_url_is_from_allowed_origins_browser_proxyAddress(self):
        with patch('streamlit.proxy.proxy_util.config.is_manually_set') as b, \
             patch('streamlit.proxy.proxy_util.config.get_option',
                   side_effect=[True, 'mybucket', 's3.amazon.com', 'browser.proxy.address']) as m:
            b.return_value = True
            self.assertTrue(url_is_from_allowed_origins('browser.proxy.address'))
