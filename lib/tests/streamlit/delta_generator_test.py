...
class DeltaGeneratorImageTest(DeltaGeneratorTestCase):
    ...
    def test_transient_on_locked_cursor_raises_error(self):
        """Test that _transient raises an error on a locked cursor."""
        from streamlit.errors import StreamlitTransientCursorError

        # Create a locked cursor DG (e.g. from an element)
        cursor = LockedCursor(root_container=RootContainer.MAIN, index=0)
        dg = DeltaGenerator(root_container=RootContainer.MAIN, cursor=cursor)

        element = ElementProto()
        element.text.body = "transient text"

        with pytest.raises(StreamlitTransientCursorError):
            dg._transient(element)
