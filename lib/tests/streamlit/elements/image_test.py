...
class ImageProtoTest(DeltaGeneratorTestCase):
    ...
    def test_image_with_link_single(self):
        ...
        assert el.imgs.imgs[0].url == img_url
        assert el.imgs.imgs[0].link == link
    ...
    def test_image_with_link_multiple(self):
        ...
        el = self.get_delta_from_queue().new_element
        assert len(el.imgs.imgs) == 2
        for idx, (url, link) in enumerate(zip(img_urls, links)):
            assert el.imgs.imgs[idx].url == url
            assert el.imgs.imgs[idx].link == link
    ...
    def test_image_with_link_mismatch(self):
        ...
        assert "Cannot pair 1 link with 2 images." in str(exc_info.value)
    ...
    def test_image_with_invalid_link(self):
        ...
        assert "The link parameter must be a valid URL" in str(exc_info.value)
