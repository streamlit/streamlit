...
class CommonCacheTest(DeltaGeneratorTestCase):
    ...
    def test_singleton_st_function_warning(self):
        """Ensure we properly warn when st.foo functions are called
        inside a cached function.
        """
        cache_decorator = singleton
        call_stack = SINGLETON_CALL_STACK
    ...
    def test_memo_st_function_warning(self):
        """Ensure we properly warn when st.foo functions are called
        inside a cached function.
        """

        cache_decorator = memo
        call_stack = MEMO_CALL_STACK
