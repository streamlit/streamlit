...
import streamlit
from streamlit.lib.credentials import Credentials, check_credentials
...
def cache_clear():
    ...
    import streamlit.lib.caching

    result = streamlit.lib.legacy_caching.clear_cache()
    cache_path = streamlit.lib.legacy_caching.get_cache_path()
    if result:
        print("Cleared directory %s." % cache_path)
    else:
        print("Nothing to clear at %s." % cache_path)

    streamlit.lib.caching.memo.clear()
    streamlit.lib.caching.singleton.clear()
