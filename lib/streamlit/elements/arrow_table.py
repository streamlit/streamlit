...
def marshall(proto, data, default_uuid):
...
def _marshall_table_caption(proto, styler):
    ...
    """
    proto.styler.caption = styler.caption if styler.caption is not None else ""
...
def _marshall_display_values(proto, df, styles):
    ...
    """
    # (HK) TODO: Some design decisions need to be made.
    new_df = _use_display_values(df, styles)
    # Reset dataframe index and columns,
    # leaving only data for ArrowTable to serialize.
    df = df.T.reset_index(drop=True)
    df = df.T.reset_index(drop=True)
    # Convert all values to strings as we
    # only use them to show the result.
    df = df.astype(str)
    proto.styler.display_values = _dataframe_to_serialized_arrow_table(df)
...
def _marshall_columns(proto, columns):
    ...
    # Convert all values to strings as we
    # only use them to show the result.
    columns_df = columns_df.astype(str)
