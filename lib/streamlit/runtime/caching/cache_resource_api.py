    ...
    def _decorator(
        ...
        *,
        ...
        refresh_type : "foreground" or "background"
            ...
            usage. In environments that forbid creating threads (for example, some
            sandboxed deployments), the refresh instead runs synchronously on the
            calling thread, so that call blocks while the resource is recomputed and
            still returns the stale resource (the fresh resource is served on the
            next call).
