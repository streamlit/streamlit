...
# The wrapped function's return type only ever appears in return position on the
# protocol below, so it is declared covariant there.
R_co = TypeVar("R_co", covariant=True)


class TaskCachedFunc(Protocol[P, R_co]):
    ...
    decorator's overloads tell callers that a call can also yield one of the
    placeholder values instead of the function's return value.
    """

    def __call__(
        self, *args: P.args, **kwargs: P.kwargs
    ) -> R_co | cache_task.Running | cache_task.TaskError: ...
...
class CachedFunc(Generic[P, R]):
    ...
    def __call__(self, *args: P.args, **kwargs: P.kwargs) -> R:
        ...
        if self._info.task:
            # Declared with task=True: hand back a placeholder rather than blocking.
            # The declared return type describes the underlying function, so the
            # decorator's overloads carry the placeholder types to callers.
            return cast("R", self._get_task_value(args, kwargs))
    ...
    def _get_task_value(
        ...
        func_args: tuple[Any, ...],
        func_kwargs: dict[str, Any],
    ) -> R | cache_task.Running | cache_task.TaskError:
        """Return this call's value, or a placeholder for one that isn't ready."""
        task = self._get_or_create_task(func_args, func_kwargs)
        if task.running:
            return cache_task.RUNNING
        if task.error is not None:
            return cache_task.TaskError(task.error)
        return task.result
