...
import os
import threading
from typing import Any, Final, ItemsView, Iterator, KeysView, Mapping, ValuesView
...
from streamlit.runtime.secrets import get_attr_dict_factory
...
class FileSecrets(Mapping[str, Any]):
    ...
    def __init__(self):
        ...
        self._file_paths = config.get_option("secrets.root")
        self._files_to_watch = []
        self._multi_file_mode = config.get_option("secrets.multipleFilesMode")
        self._attr_dict_factory = get_attr_dict_factory(
            _missing_attr_error_message, _missing_key_error_message
        )

        self.change_listener = Signal(
    ...
    def _parse_single_file(self, print_exceptions):
        ...
        for path in self._file_paths:
            try:
                ...
                self._files_to_watch.append(path)
            except FileNotFoundError:
                continue
    ...
    def _parse_multiple_files(self, print_exceptions):
        secrets = {}
        found_secrets_file = False

        for path in self._file_paths:
            for dirname in os.listdir(path):
                if not dirname.startswith("secret_"):
                    continue
                key = dirname[7:]  # Remove "secret_" prefix
                sub_secrets = {}
                for filename in os.listdir(os.path.join(self._secrets_dir, dirname)):
                    file_path = os.path.join(self._secrets_dir, filename)
                    with open(file_path, "r") as f:
                        sub_secrets[filename] = f.read().strip()
                        self._files_to_watch.append(file_path)

                if len(sub_secrets) == 0:
                    continue

                if len(sub_secrets) == 1:
                    secrets[key] = sub_secrets[sub_secrets.keys()[0]]
                else:
    ...
    def _parse(self, print_exceptions: bool) -> Mapping[str, Any]:
        ...
        with self._lock:
            ...
            if self._multi_file_mode:
                secrets = self._parse_multiple_files(print_exceptions)
            else:
                secrets = self._parse_single_file(print_exceptions)
    ...
    def __getattr__(self, key: str) -> Any:
        ...
        try:
            ...
            if not isinstance(value, Mapping):
                ...
                return self._attr_dict_factory(value)
