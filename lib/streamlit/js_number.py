...
class JSNumber(object):
    ...
    def validate_int_bounds(cls, value, value_name=None):
        ...
        if not isinstance(value, int):
            raise JSNumberBoundsException("%s is not an int" % value_name)
        elif value < cls.MIN_SAFE_INTEGER:
            raise JSNumberBoundsException("%s must be >= -((1 << 53) - 1)" % value_name)
    ...
    def validate_float_bounds(cls, value, value_name):
        ...
        if not isinstance(value, (int, float)):
            raise JSNumberBoundsException("%s is not a float" % value_name)
        elif value < cls.MIN_NEGATIVE_VALUE:
            raise JSNumberBoundsException("%s must be >= -1.797e+308" % value_name)
