...
Prefer a reserved `st.container()` here over standalone placeholders you fill after slow work. A standalone `st.skeleton()` re-sends its skeleton at the top of the rerun, so the old element unmounts and loses its state before the slow fill remounts it. A refilled `st.empty()` avoids this — on a rerun its re-sent placeholder keeps the previous element mounted and the later same-position fill reconciles in place, preserving a dataframe's scroll, sort, and selection (an explicit same-run `placeholder.empty()` still clears immediately, and an `st.empty()` that is never refilled is cleared as stale at end of run). Even so, a container is cleaner because it shows a skeleton while loading instead of greyed, stale content. Give stateful elements a stable `key` — without one, a dataframe's identity includes its data, so it remounts when the data changes. See `layouts.md` for placeholder details.
