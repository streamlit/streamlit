...
A refilled standalone placeholder (`st.empty()` or `st.skeleton()`) keeps its element mounted across reruns: the re-sent placeholder preserves the previous element and the later same-position fill reconciles in place, preserving a dataframe's scroll, sort, and selection (an explicit same-run `placeholder.empty()` still clears immediately, and a placeholder that is never refilled is cleared as stale at end of run). Even so, a reserved `st.container()` is cleaner for slow work because `container.skeleton()` shows a skeleton animation while loading instead of greyed, stale content. Give stateful elements a stable `key` — without one, a dataframe's identity includes its data, so it remounts when the data changes. See `layouts.md` for placeholder details.
