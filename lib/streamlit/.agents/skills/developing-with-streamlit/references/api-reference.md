...
| `ThemeConfig` | Mapping constructed by the user for `st.container(theme=...)` and `st.set_page_config(theme=...)` |
| `ThemeVariantConfig` | Nested `light` / `dark` mapping constructed by the user inside `ThemeConfig` |
