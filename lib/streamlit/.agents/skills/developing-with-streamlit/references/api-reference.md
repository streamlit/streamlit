...
| `st.pyplot` | Display a Matplotlib `Figure` (`fig` is required). Use it for existing Matplotlib visualizations or libraries that produce Matplotlib figures. Calling without `fig` (global Matplotlib state) is not supported. |
