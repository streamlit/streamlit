...
`persist_state` requires a `key` and is available on the input, selection, and slider widgets (the same widgets that support `bind="query-params"`). When both are set, `bind` takes precedence, so the value lives in the URL and persists across page switches regardless of the `persist_state` scope.
