...
`st.text_input`'s `validate` parameter is one input rule that *can* run on the server: passing a **callable** (instead of a client-side regex string) executes the check on the backend, so its logic stays hidden from the browser. Prefer a callable when the validation logic must stay server-side or needs backend resources. It is still **not** a security boundary, though: the callable only runs when the browser requests it, so a modified client can submit a value directly without triggering the check. Keep re-validating security-critical values in your script as shown below.
