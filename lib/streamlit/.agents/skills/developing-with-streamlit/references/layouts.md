...
| `st.expander`     | You need optional details, advanced settings, explanations, or diagnostic output that should not dominate the main view. Content is computed even when collapsed; for lazy execution where content runs only when open, use `on_change="rerun"` (or a callable) or `bind="query-params"` (with `key`), then check `.open`. |
