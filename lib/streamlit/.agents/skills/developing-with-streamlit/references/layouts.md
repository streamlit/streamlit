...
| `st.form`         | You need to batch multiple widget inputs and rerun only when the user submits. For `st.text_input` and `st.slider`, `on_change="ignore"` is an alternative that suppresses that widget's rerun without a form.                                                                               |
