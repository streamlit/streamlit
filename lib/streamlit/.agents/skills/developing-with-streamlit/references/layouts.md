...
| `st.expander`     | You need optional details, advanced settings, explanations, or diagnostic output that should not dominate the main view. Use `type="compact"` for a borderless inline toggle or `type="step"` for consecutive timeline steps; see `chat-ui.md`.                                             |
| `st.status`       | You need to show progress, logs, or multi-step work in a collapsible status block that can update from running to complete or error. Same `type="compact"` and `type="step"` options as `st.expander`.                                                                                      |
