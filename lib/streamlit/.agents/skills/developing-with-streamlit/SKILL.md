...
- Prefer Material Symbols icons (`:material/icon_name:`) over emojis for navigation, buttons, and labels. Use emojis sparingly, only when they add a special touch. Only some commands take an `icon` parameter; headings and other text do not, so write the directive in the string instead: `st.title(":material/analytics: Sales dashboard")`.
