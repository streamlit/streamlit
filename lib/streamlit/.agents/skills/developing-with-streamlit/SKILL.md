...
| **Rich text formatting** — Markdown in `st.markdown` and widget labels, colored text (`:red[...]`), badges, Material Symbols icons (`:material/icon_name:`), LaTeX math, Mermaid diagrams, and `wrap=False` to keep markdown, captions, headings, and text on one ellipsized line | read `references/markdown.md` |
