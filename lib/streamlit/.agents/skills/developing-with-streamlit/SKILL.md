...
- Do not pass Matplotlib `savefig` kwargs to `st.pyplot`; they are deprecated. `st.pyplot` already uses `bbox_inches="tight"` and `dpi=200`. For other `savefig` options, use `fig.savefig` + `st.image`.
