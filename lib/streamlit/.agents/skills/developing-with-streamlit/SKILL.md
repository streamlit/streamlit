...
- Use `st.form` to batch related inputs and rerun only on submit, especially when intermediate widget changes would trigger expensive work. For `st.text_input` and `st.slider`, `on_change="ignore"` also suppresses that widget's rerun until something else triggers one.
...
| **Persisting values across reruns** — `st.session_state`, widget keys, callbacks (`on_change`, `on_click`), `on_change="ignore"` on `st.text_input`/`st.slider`, and patterns for stateful interactions | read `references/session-state.md` |
