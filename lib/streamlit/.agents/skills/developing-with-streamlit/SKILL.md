...
| **Custom themes, colors, and CSS styling** — app-wide colors in `.streamlit/config.toml`, reading the active theme via `st.context.theme`, and styling a single widget with scoped CSS (`key=` + `.st-key-{key}` in `st.html()`, never a global selector) | read `references/theme.md` |
| **Plotting points on a geographic map** — scatter points by latitude/longitude with `st.map(df, latitude=, longitude=, size=, color=)`; use the `size=` arg (column name, or a fixed value in meters) to scale marker radius. Prefer `st.map` over hand-rolled `pydeck`/`st.pydeck_chart` for simple point maps | read `references/data-display.md` |
