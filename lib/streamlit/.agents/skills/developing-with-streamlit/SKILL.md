...
| **Persisting values across reruns** — `st.session_state`, widget keys, callbacks (`on_change`, `on_click`), `st.rerun()` / `st.switch_page()` from callbacks, and patterns for stateful interactions | read `references/session-state.md` |
