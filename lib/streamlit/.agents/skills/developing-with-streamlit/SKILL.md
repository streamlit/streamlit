...
| **Rich text formatting** — Markdown in `st.markdown` and widget labels, colored text (`:red[...]`), badges, Material Symbols icons (`:material/icon_name:`), LaTeX math, and Mermaid diagrams (` ```mermaid ` code blocks or `st.mermaid_chart`) | read `references/markdown.md` |
