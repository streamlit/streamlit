...
| **Discovering available Streamlit public APIs, looking up `st.<command>` commands, exact parameters, docstrings, signatures, or choosing the right top-level command** — quick tables of public `st` commands and related namespaces plus CLI instructions for inspecting local docstrings | read `references/api-reference.md` |
