...
- Order scripts so fast UI (titles, layout, widgets) renders before slow computation. Streamlit streams elements top to bottom and greys out (marks stale) everything below an in-progress slow step until the rerun finishes; put slow work last, reserve slots with `st.empty()` or `st.skeleton()`, or isolate slow sections in fragments.
