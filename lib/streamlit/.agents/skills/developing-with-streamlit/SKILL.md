...
- Do not apply CSS to style the app unless the user actively requests it. Use native Streamlit features, `.streamlit/config.toml`, `st.set_page_config(theme=...)`, and `st.container(theme=...)` to customize the appearance; see the [theming reference](references/theme.md).
...
| **Custom themes, colors, or styling requests** — configuring colors in `.streamlit/config.toml`, runtime overlays with `st.set_page_config(theme=...)`, scoped restyling with `st.container(theme=...)`, reading the active theme via `st.context.theme`, and the CSS pattern to use only when the user explicitly asks for CSS | read `references/theme.md` |
