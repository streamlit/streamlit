...
| **Buttons and trigger widgets** — firing an action on click (`st.button`), a one-shot action/toolbar menu (`st.menu_button`, e.g. an Export menu offering CSV/Excel/PDF), sending a file (`st.download_button`), or linking to a URL (`st.link_button`) | read `references/trigger-widgets.md` |
