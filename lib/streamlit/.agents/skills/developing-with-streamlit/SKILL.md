...
| **Value-entry input widgets** — `st.number_input` (with bounds), `st.slider`, `st.date_input`/`st.time_input`/`st.datetime_input`, `st.text_input`/`st.text_area`; picking the right widget for numbers/dates/text instead of `st.text_input` + casting | read `references/input-widgets.md` |
