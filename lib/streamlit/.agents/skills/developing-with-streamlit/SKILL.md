...
| **Plotting points on a geographic map** — scatter points by latitude/longitude with `st.map(df, latitude=, longitude=, size=, color=)`; use the `size=` arg (column name or pixel value) to scale marker radius. Prefer `st.map` over hand-rolled `pydeck`/`st.pydeck_chart` for simple point maps | read `references/data-display.md` |
