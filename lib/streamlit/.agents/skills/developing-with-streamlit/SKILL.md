...
- Order scripts so fast UI (titles, layout, widgets) renders before slow computation. Streamlit streams elements top to bottom and temporarily greys out (marks stale) not-yet-redrawn elements from the previous run while a slow step is in progress, clearing each as the new run recreates it; put slow work last, reserve output slots with `st.container()` or wrap slow work in a `with st.skeleton()` block (both keep contents mounted, unlike standalone `st.empty`/`st.skeleton`, which can unmount and reset their contents when slow work runs before the fill), or isolate slow sections in fragments.
