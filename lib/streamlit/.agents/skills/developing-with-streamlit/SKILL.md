...
- Use `st.fragment` for independent sections that should rerun separately from the rest of the app, such as auto-refreshing charts or controls that do not need to rerun the full page. When the triggering widget lives *outside* the fragment, use `@st.fragment(key=...)` with `st.rerun("<key>")` in a callback to rerun only that fragment.
