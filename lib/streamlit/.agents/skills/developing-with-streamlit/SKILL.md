...
| **Custom themes, colors, or styling requests** — configuring colors in `.streamlit/config.toml`, reading the active theme at runtime via `st.context.theme`, and (only when the user explicitly asks for CSS) scoped `.st-key-<key>` selectors via `st.html` | read `references/theme.md` |
