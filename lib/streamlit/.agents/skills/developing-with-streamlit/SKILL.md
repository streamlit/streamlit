...
| **Looking up Streamlit public API commands, exact `st.<command>` parameters, docstrings, signatures, or choosing the right top-level command** — quick table of public `st` commands plus CLI instructions for inspecting local docstrings | read `references/api-reference.md` |
