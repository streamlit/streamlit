...
FOLDERS = [
    os.path.abspath(os.path.normpath(x)) for x in sys.argv[1:]
]
...
HEADER_TEMPLATE = (
f'''%(open_header)s%(comment)s Copyright 2018-{THIS_YEAR} Streamlit Inc.
%(comment)s
...
for glob_pattern, comment_style in glob_to_comment_style.items():
    ...
    for filename in filenames:
        ...
        old_header_start, old_header_end = get_header_bounds(
            lines, comment_style)
