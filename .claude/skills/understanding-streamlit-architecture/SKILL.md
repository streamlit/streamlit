...
- **Always pass identity-affecting kwargs, including `None` when the feature is off.** Upgrading Streamlit already wipes session state, so omitting a kwarg to keep a pre-upgrade hash is not useful.
