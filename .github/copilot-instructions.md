...
- Select one final Python environment by setting `PYTHON_DEPENDENCY_GROUP` to `runtime`, `test`, `dev`, or `integration`, then run `make python-init` (`dev` is the default). This exact-syncs the shared `.venv`; use separate `UV_PROJECT_ENVIRONMENT` paths when simultaneous environments are needed.
