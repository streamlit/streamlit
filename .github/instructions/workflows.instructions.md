...
| Workflow | Trigger | Description |
|----------|---------|-------------|
| `ai-pr-review.yml` | `ai-review`/`ai-final-review` label or manual | AI-powered code and product-alignment review using Cursor CLI. `ai-final-review` auto-approves on an `APPROVED` verdict except for `change:spec` or `change:feature`+`impact:users` PRs, PRs over 1000 changed lines, or PRs whose HEAD moved since the agents reviewed them |
