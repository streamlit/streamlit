...
If an error is encountered an exception is raised. If the error occurs while hashing either the key or the output an `UnhashableTypeError` error is thrown. If you run into any issues, see [fixing caching issues](troubleshooting/caching-issues.md).
...
| `hash_func` | Speed | Use case |  
|-------------|-------|----------|  
| Python's [id](https://docs.python.org/3/library/functions.html#id) function. <br />[Example](#example-1-pass-a-database-connection-around) | Fast | If you're hashing a singleton object, like an open database connection or a TensorFlow session. These are objects that will only be instantiated once, no matter how many times your script reruns. |  
| `lambda _: None` <br />[Example](#example-2-turn-off-hashing-for-a-specific-type) | Fast | If you want to turn off hashing of this type. This is useful if you know the object is not going to change. |  
| Python's [`hash()`](https://docs.python.org/3/library/functions.html#hash) function <br />[Example](#example-3-use-python-s-hash-function) | Can be slow based the size of the object being cached | If Python already knows how to hash this type correctly. |  
| Custom hash function <br />[Example](#the-hash-funcs-parameter) | n/a | If you'd like to override how Streamlit hashes a particular type. |
...
Sometimes, you might want to use Python’s default hashing instead of Streamlit's. For example, maybe you encountered a type that Streamlit is unable to hash, but is hashable with Python's built-in `hash()` function. In that case, the solution is quite simple:
