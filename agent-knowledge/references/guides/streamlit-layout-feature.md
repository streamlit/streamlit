...
with st.container(key="vertical_parent_container", height=400):
  # The text area and markdown elements should share the vertical space equally.
  st.text_area("Enter your message here", height="stretch")
  # The markdown element will also stretch to share remaining space.
  st.markdown("Additional content below", height="stretch")
```
...
Some elements have this mode which indicates customized behaviour for the element.

Examples:

- st.text_area has this height mode. When it is selected, the text area element expands as text lines are entered in the input.
