...
class Builder {
  ...
  get byteLength() {
    ...
    return this.children.reduce(
      (size, child) => size + child.byteLength,
      size
    );
  }
  /**
  ...
  flush() {
    ...
    if (typeIds) {
      ...
      // Binary, Utf8
      values &&
        (buffers[enum_1.BufferType.DATA] = values.flush(offsets.last()));
    ...
    nullCount > 0 &&
      (buffers[enum_1.BufferType.VALIDITY] = this._nulls.flush(length));
...
class FixedWidthBuilder extends Builder {
  constructor(opts) {
    super(opts);
    this._values = new buffer_1.DataBufferBuilder(
