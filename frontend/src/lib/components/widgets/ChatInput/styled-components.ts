...
export const StyledSendIconButton = styled.button<StyledSendIconButtonProps>(
  ({ theme, disabled, extended }) => {
    ...
    return {
      border: "none",
      backgroundColor: theme.colors.transparent,
      borderTopRightRadius: extended ? theme.radii.none : theme.radii.md,
      borderTopLeftRadius: extended ? theme.radii.md : theme.radii.none,
...
export interface StyledFloatingChatInputContainerProps {
  width: number
}

export const StyledFloatingChatInputContainer =
  styled.div<StyledFloatingChatInputContainerProps>(({ theme, width }) => ({
    position: "fixed",
    bottom: "0px",
    paddingBottom: "70px",
