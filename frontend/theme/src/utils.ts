...
export const parseFont = (
  ...
  // "sans-serif".
  const fontKey = trimmedFont.toLowerCase().replace(/ /g, "-")
...
const setHeadingFontSizes = (
  ...
  if (headingFontSizes) {
    headingFontSizes.forEach((size, index) => {
      ...
      if (convertedSize) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- dynamic key access for h1-h6 font sizes
        ;(headingFontSizesOverrides as any)[headingFontSizeKey] = convertedSize
      }
    })
  }
  ...
  if (headingFontWeights) {
    ...
    headingFontWeights.forEach((weight, index) => {
      ...
      if (
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- dynamic key access for h1-h6 font weights
        ;(fontWeightOverrides as any)[headingFontWeightKey] = weight
      }
    })
  }
...
export const createCustomThemes = (
  ...
  if (hasLightConfigs || hasDarkConfigs) {
    ...
    customThemes.push(lightThemeConfig, darkThemeConfig, autoTheme)
  } else {
...
export const createSidebarTheme = (activeTheme: ThemeConfig): ThemeConfig => {
  ...
  if (activeTheme.name === CUSTOM_THEME_LIGHT_NAME) {
    baseThemeType = CustomThemeConfig.BaseTheme.LIGHT
  } else if (activeTheme.name === CUSTOM_THEME_DARK_NAME) {
    baseThemeType = CustomThemeConfig.BaseTheme.DARK
  }
