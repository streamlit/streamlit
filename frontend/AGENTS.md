...
- `make frontend-knip`: Run Knip unused-export and unused-dependency analysis. Unused deps fail; `unlisted` stays off because some imports resolve from nested packages. `ignoreDependencies` covers packages Knip cannot see; dummy scripts (`buf`, `playwright-cli`, `pbjs`, `pbts`) exist so Knip treats those binaries as used.
