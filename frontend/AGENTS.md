...
- **Use theme properties**: Always use theme properties from `useEmotionTheme()` instead of hardcoded values. The theme provides `colors`, `spacing`, `sizes`, `radii`, `fontSizes`, `fontWeights`, `lineHeights`, `shadows`, `iconSizes`, `zIndices`, and more. See `frontend/lib/src/theme/types.ts` for the full `EmotionTheme` type.
