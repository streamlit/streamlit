...
- **Name refs with a `Ref`/`ref` suffix**: Variables assigned from `useRef(...)` must end with `Ref` (preferred) or `ref`. This is enforced by `@eslint-react/naming-convention/ref-name`.
