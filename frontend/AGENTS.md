...
- **Do not use deprecated APIs**: `@typescript-eslint/no-deprecated` is an error. For protobuf messages, use `Type.$Properties` for plain objects (e.g. `Alert.$Properties`) instead of the generated `IType` aliases (`IAlert`). The message class remains correct for constructed or decoded instances.
