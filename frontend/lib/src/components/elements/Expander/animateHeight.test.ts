...
describe("animateHeight", () => {
  ...
  describe("returned handle", () => {
    it("returns an object with cancel and finished", () => {
      const handle = animateHeight(element, 0, 100)

      expect(handle).toHaveProperty("cancel")
      expect(handle).toHaveProperty("finished")
    ...
    it("handles environments where animation.cancel is not a function", () => {
      mockAnimation.cancel = undefined as unknown as ReturnType<typeof vi.fn>
      const handle = animateHeight(element, 0, 100)
