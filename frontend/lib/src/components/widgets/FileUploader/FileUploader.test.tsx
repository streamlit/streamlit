...
describe("FileUploader widget tests", () => {
  ...
  it("uploads a single file upload", async () => {
    ...
    await waitFor(() => user.upload(fileDropZoneInput, fileToUpload))
  ...
  it("replaces file on single file uploader", async () => {
    ...
    await waitFor(() => user.upload(fileDropZoneInput, firstFile))
    ...
    await waitFor(() => user.upload(fileDropZoneInput, secondFile))
  ...
  it("can delete completed upload", async () => {
    ...
    // Upload two files
    await act(() =>
      user.upload(fileDropZoneInput, createFile("filename1.txt"))
    )
    await act(() =>
      user.upload(fileDropZoneInput, createFile("filename2.txt"))
    )

    const fileNames = await screen.findAllByTestId("stUploadedFile")
    ...
    await waitFor(() => user.click(within(firstDeleteBtn).getByRole("button")))
  ...
  it("can delete in-progress upload", async () => {
    ...
    await waitFor(() => user.upload(fileDropZoneInput, createFile()))
    ...
    await waitFor(() => {
      user.click(within(deleteBtn).getByRole("button"))
