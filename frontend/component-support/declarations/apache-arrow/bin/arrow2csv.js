...
async function* recordBatchReaders(createSourceStream) {
  ...
  if (!readers) {
    ...
    try {
      for await (reader of Arrow_node_1.RecordBatchReader.readAll(
    ...
    transform(batch, _enc, cb) {
      ...
      if (batch.length > 0 && batch.numCols > 0) {
        ...
        for (const row of batch) {
          ...
          this.push(
            `${formatRow(
              [rowId, ...row.toArray()].map(pretty_1.valueToString),
              maxColWidths,
              sep
            )}\n`
          )
  ...
  for (const row of batch) {
    ...
    for (val of row) {
      if (
        ...
        maxColWidths[j + 1] = Math.max(
          maxColWidths[j + 1] || 0,
          pretty_1.valueToString(val).length
        )
      }
      ++j
    }
