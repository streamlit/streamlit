...
def test_multiselect_show_values_in_dropdown(
    ...
    for idx, el in enumerate(dropdown_elems):
        assert_snapshot(el.screenshot(), name="multiselect-dropdown-" + str(idx))
...
def test_multiselect_long_values_in_dropdown(
    ...
    for idx, el in enumerate(dropdown_elems):
        assert_snapshot(
            el.screenshot(), name="multiselect-dropdown-long-label-" + str(idx)
        )
...
def test_multiselect_single_selection(app: Page, assert_snapshot: ImageCompareFunction):
    ...
    assert_snapshot(
        app.locator(".stMultiSelect").nth(1).screenshot(), name="multiselect-selection"
    )
