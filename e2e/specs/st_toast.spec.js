...
describe("st.toast", () => {
  ...
  themes.forEach((theme) => {
    it(`displays default toast correctly - ${theme}`, () => {
      ...
      cy.getIndexed("[data-testid='stToast']", 1).matchImageSnapshot(
        `toast-default-${theme}`
      );
    });

    it(`displays long message toast correctly - ${theme}`, () => {
      cy.getIndexed("[data-testid='stMarkdownContainer']", 0).should(
        "contain.text",
        "Random toast message that is a really really really really really really really long message, going way"
      );

      cy.getIndexed("[data-testid='stToast']", 0).matchImageSnapshot(
        `toast-long-${theme}`
      );
    });
