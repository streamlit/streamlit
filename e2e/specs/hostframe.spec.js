...
// TODO(vdonato): Migrate this test to playwright
describe("Host communication", () => {
  before(() => {
    cy.visit("hostframe.html");
    cy.get("#toolbar button").should("have.length", 12);
  ...
  it("handles host terminate and restart websocket connection messages", () => {
    // Kill the websocket connection and verify that the app moves into an
    // error state.
    cy.get("#toolbar").contains("Terminate Websocket").click();
    cy.get("iframe").iframe(() => {
      cy.get("[data-testid='stStatusWidget']").should("have.text", "Error");
    });

    // Request that the websocket connection gets restarted.
    cy.get("#toolbar").contains("Restart Websocket").click();

    // Check that the status widget appears and disappears as the app reruns
    // and completes.
    cy.get("iframe").iframe(() => {
      cy.get("[data-testid='stStatusWidget']").should("exist");
      cy.get("[data-testid='stStatusWidget']").should("not.exist");
